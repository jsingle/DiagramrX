...
Raphael.g = {
    ...
    snapEnds: function(from, to, steps) {
        ...
        if (r) {
            while (R) {
                i--;
                R = ~~(d * Math.pow(1000, i)) / Math.pow(1000, i);
            }
        ...
        t = round(to * Math.pow(1000, i)) / Math.pow(1000, i);

        if (t < to) {
            t = round((to + .5) * Math.pow(1000, i)) / Math.pow(1000, i);
        }

        f = round((from - (i > 0 ? 0 : .5)) * Math.pow(1000, i)) / Math.pow(1000, i);
